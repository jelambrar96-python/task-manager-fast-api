...
async def test_superuser_task_comments_flow():
    async with AsyncClient(base_url="http://localhost:8000") as ac:
        ...
        task_response = await ac.post("/tasks/", headers=auth_header, json=task_payload)
        ...
        comment1_response = await ac.post(f"/tasks/{task_id}/comments", headers=auth_header, json=comment_payload1)
        comment2_response = await ac.post(f"/tasks/{task_id}/comments", headers=auth_header, json=comment_payload2)
        ...
        single_comment_response = await ac.get(f"/tasks/{task_id}/comments/{comment1['id']}", headers=auth_header)
        ...
        # 5. Eliminar comment1
        delete_response = await ac.delete(f"/tasks/{task_id}/comments/{comment1['id']}", headers=auth_header)
        ...
        get_deleted_response = await ac.get(f"/tasks/{task_id}/comments/{comment1['id']}", headers=auth_header)        
        ...
        update_response = await ac.put(f"/tasks/{task_id}/comments/{comment2['id']}", headers=auth_header, json=update_payload)
        ...
        confirm_response = await ac.get(f"/tasks/{task_id}/comments/{comment2['id']}", headers=auth_header)
